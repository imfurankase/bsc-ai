...
const AttachmentCard = ({
  ...
  if (attachment.url) {
    ...
    return <div onClick={handlePreview}>{CardContent}</div>;
  }
...
export const MessageBubble = ({
  ...
  const hasChart = !!message.chartData && message.chartData.rows.length > 0;
  ...
  return (
    ...
    >
      ...
      >
            ...
            {isUser ? (
              ...
              <div className="prose prose-sm prose-invert max-w-none [&>p]:mb-4 [&>p:last-child]:mb-0 [&>ul]:my-3 [&>ul]:ml-0 [&>ul>li]:mb-1.5">
                <ReactMarkdown
                  components={{
                    p: ({ children }) => (
                      <p className="leading-relaxed text-[hsl(35,20%,85%)] text-[15px]">
                        {children}
                      </p>
                    ),
                    strong: ({ children }) => (
                    ...
                    ul: ({ children }) => (
                      <ul className="space-y-1.5 list-disc list-inside text-[hsl(35,20%,85%)]">
                        {children}
                      </ul>
                    ),
                    li: ({ children }) => (
                      <li className="text-[hsl(35,20%,85%)] text-[15px]">
                        {children}
                      </li>
                    ),
                  }}
        ...
        {/* Inline chart for assistant messages (from tabular analysis) */}
        {!isUser && hasChart && message.chartData && (
          <div className="mt-3 rounded-2xl border border-white/10 bg-black/30 p-3">
            {message.chartData.title && (
              <p className="text-xs text-white/60 mb-2">
                Chart from: {message.chartData.title}
              </p>
            )}
            <div className="w-full h-56">
              <ChartContainer
                config={{
                  ...(message.chartData.y_keys || []).reduce(
                    (acc, key, idx) => {
                      const colors = [
                        "hsl(221,83%,53%)",
                        "hsl(142,76%,36%)",
              ...
              >
                <LineChart data={message.chartData.rows as any[]}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey={message.chartData.x_key} />
                  <YAxis />
                  <ChartTooltip content={<ChartTooltipContent />} />
                  <ChartLegend content={<ChartLegendContent />} />
                  {message.chartData.y_keys.map((key, idx) => (
                    <Line
                      key={key}
                      type="monotone"
                      dataKey={key}
