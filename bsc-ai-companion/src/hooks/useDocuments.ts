...
export const useDocuments = () => {
   ...
   */
  const loadDocuments = useCallback(async (conversationId?: number): Promise<Document[]> => {
   ...
   */
  const getDocumentStatus = useCallback(async (documentId: number): Promise<DocumentStatusResponse | null> => {
    const { data, error } = await api.get<DocumentStatusResponse>(`/api/documents/${documentId}/status/`);
   ...
   */
  const linkDocumentToConversation = useCallback(async (
    documentId: number,
    conversationId: number
  ): Promise<boolean> => {
   ...
   */
  const getDocumentContext = useCallback(async (conversationId: number): Promise<string> => {
    ...
    console.log('Document context requested for conversation:', conversationId);
    return '';
  }, []);
