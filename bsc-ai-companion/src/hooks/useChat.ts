...
export const useChat = () => {
  ...
  const loadConversations = useCallback(async (): Promise<ConversationSummary[]> => {
    const { data, error } = await api.get<ConversationsListResponse>('/api/conversations/');
  ...
  const loadConversation = useCallback(async (conversationId: number): Promise<ConversationDetail | null> => {
    const { data, error } = await api.get<ConversationResponse>(`/api/conversations/${conversationId}/`);
   ...
   */
  const loadMessages = useCallback(async (conversationId: number): Promise<Message[]> => {
  ...
  const createConversation = useCallback(async (
    ...
    const { data, error } = await api.post<ConversationResponse>('/api/conversations/', {
      title,
      chat_type: chatType,
    });
   ...
   */
  const updateConversationTitle = useCallback(async (conversationId: number, title: string): Promise<boolean> => {
    const { error } = await api.patch<ConversationResponse>(`/api/conversations/${conversationId}/`, {
      title,
    });
  ...
  const deleteConversation = useCallback(async (conversationId: number): Promise<boolean> => {
    const { error } = await api.delete(`/api/conversations/${conversationId}/`);
